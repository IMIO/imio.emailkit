...
def _compile(domain: str):
    ...
    for po_file in sorted(locale_path.glob(f"*/LC_MESSAGES/{domain}.po")):
        compile_mo_file(domain, str(po_file.parent))
        logger.info(f"Compiled {po_file.parent}/{domain}.mo")
