...
Tested through the real call site, with the same keyword arguments
``RegistrationTool.py`` passes. Checks both the resolved file and the
rendered output: the file alone can be correct while the stock template
still renders.
"""
...
def marked_request(portal, http_request):
    """A request carrying ``IEmailkitLayer``, applied by hand since an
    integration test never traverses to trigger it."""
...
class TestWeOwnTheStockViews:
    """The stock view names must resolve to our classes: if not, every test
    below would still pass while stock Plone's mail went out instead."""
    ...
    def test_it_renders_a_registered_template(self, view):
        """Goes through discovery and ``render()`` like any consumer
        template."""
    ...
    def test_stock_kwargs_are_what_we_build_from(self, method, expected):
        """A Plone upgrade renaming a kwarg would silently give
        ``build_context`` ``None`` and ship an empty link."""
        import inspect
...
class TestRenderedOutputIsOurs:
    def test_kit_accessibility_default_is_present(self, rendered):
        """The cheapest proof that a kit template rendered."""
    ...
    def test_css_was_inlined(self, rendered):
        """A placeholder in a literal ``style`` attribute can silently
        break CSS inlining."""
    ...
    def test_no_placeholder_survived(self, rendered):
        """Under the ``zope.tal`` fallback, ``${...}`` reaches the inbox
        unrendered and nothing raises."""
        support.assert_render_is_clean(rendered, "the rendered mail")

    def test_stock_body_is_gone(self, rendered, template):
        """Checked against the English text, not the msgid, which would
        never fail."""
    ...
    def test_the_mail_says_whose_account_it_is_about(self, rendered, member):
        """Either the fullname or the userid; a mail naming neither looks
        like phishing."""
...
class TestThemeTokensReachTheDefaultMails:
    """The stock view's namespace has no ``theme`` key, so a token wired
    only into ``render()``'s namespace would silently do nothing here."""
    ...
    def test_the_primary_color_reaches_the_default_mail_render(
        ...
        assert with_probe_color in rendered, (
            "the theme token did not reach the render of the two mails :default ships"
        )


class TestTheResultIsStillAParsableMail:
    """A restyled body that loses the headers sends a mail to nobody, from
    ...
    def test_the_subject_is_our_registered_msgid_translated(
        ...
        """Checked against the translation of the registered msgid, not a
        list of stock strings: an identical English default proves nothing.
        """
    ...
    def test_a_content_type_is_declared(self, rendered):
        """Without one, the mail defaults to ``text/plain`` and clients
        show the raw markup."""
        message = message_from_string(rendered.strip())
