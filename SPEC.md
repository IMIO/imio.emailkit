...
- **All three are overridden at the *view*, not at the template.** `imio.emailkit` registers its own `mail_password_template`, `registered_notify_template` and `login-help` views, under the same names, `for` and permission as stock, differing only by layer. Each builds a flat context and renders a registered template through §6.1 `render()`. The username reminder was always this way — stock Plone sends it as a hardcoded plaintext string (`SEND_USERNAME_TEMPLATE` in `Products/CMFPlone/browser/login/login_help.py`), so there was no file for `z3c.jbot` to key on and the view was the only seam. The other two *could* be reached with jbot and were, until it became clear what that cost: a stock view renders them, so their bodies had to speak that view's dialect (`options/…`, `python:member.getProperty(…)`, no locale helpers, no `theme`), and they could not be registered for discovery — so they never appeared in `bin/preview-emails` or `@@emailkit-preview`, and had no golden files. Owning the view removes the exception instead of documenting it, and this section's "exactly like consumer templates" is now true of *every* verb for *every* default mail.
- The one piece of Plone machinery that cannot move into the template is the header block: `RegistrationTool` parses `Subject`/`To`/`From`/`Content-Type` back out of what the view returns. It is built in Python, from data, in `browser/default_mails.py` — so a subject is a msgid in a §4 registration like every other subject, rather than markup on a hand-written `Subject:` line.
- The three **view registrations** are bound **to a dedicated browser layer** (`IEmailkitLayer`) installed by the `imio.emailkit:default` GenericSetup profile. This package registers **no `browser:jbot` directory**; it still includes the whole `z3c.jbot` package, because §4's promise that a consumer can override the `.pt` files *we* resolve depends on those patches.
...
1. **Replace the template markup per site/client:** register a jbot directory on a *more specific* browser layer (the site package's own layer). z3c.jbot layer precedence applies — the most specific layer wins. This is the standard iMio pattern already used elsewhere; no new mechanism. What a site overrides is **our** compiled template (`imio.emailkit.templates.<name>.pt`), for a default mail exactly as for any other — one override story, one dialect, one filename convention. What is layer-bound rather than jbot-bound is the **view** that sends each mail.
