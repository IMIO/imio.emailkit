 ...
 * local account has been migrated onto its Wallonie Connect (SSO) identity.
 *
 * Context keys:
 ...
 *
 * The subject names no product, because the registration subject is one fixed
 * msgid for every consumer. The product name is in the lead paragraph instead.
 */
</script>
...
<template>
  <KitMain>
    ...
    <!--
      The mark, then the button that repeats it in words. `asset_base` is empty
      ...
      A raster logo, dark on a white plate: mail clients do not render SVG, and a
      white-on-colour copy of this mark would vanish into the content well.
      Blocked images cost nothing here ; the `alt` and the button below say the
      same thing.
    -->
