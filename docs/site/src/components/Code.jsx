  ...
  return (
    <button
      ...
      className={clsx(
        ...
        copied
          ? 'bg-accent-400/10 ring-1 ring-accent-400/20 ring-inset'
    ...
    >
      ...
      <span
        aria-hidden={!copied}
        className={clsx(
          'pointer-events-none absolute inset-0 flex items-center justify-center text-accent-400 transition duration-300',
          !copied && 'translate-y-1.5 opacity-0',
        )}
      >
        Copied!
...
function CodePanel({ children, tag, label, code }) {
  // Read `tag`/`label`/`code` off the child when the child is a plain element.
  //
  // The template shipped `Children.only(children)` here. That invariant is not
  // one this component needs: for a fenced code block the rehype plugin already
  // puts `code` (and `language`) on the panel itself, and the child is only
  // consulted for the hand-authored `<CodeGroup><Code code=… /></CodeGroup>`
  // form. But webpack is free to hand a client-component child over as a lazy
  // module reference rather than an element, and `Children.only` rejects those —
  // which crashed the prerender of one page whose markup was byte-identical to a
  // page that built fine, purely because the chunk boundaries fell differently.
  // So: extract props opportunistically, and never assert on the child's shape.
